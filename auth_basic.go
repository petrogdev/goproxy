...
func Basic(realm string, f func(user, passwd string) bool) ReqHandler {
	return FuncReqHandler(func(req *http.Request, ctx *ProxyCtx) (*http.Request, *http.Response) {
		if !auth(req, f) {
...
//
// You probably want to use auth.ProxyBasic(proxy) to enable authentication for all proxy activities
func BasicConnect(realm string, f func(user, passwd string) bool) HttpsHandler {
	return FuncHttpsHandler(func(host string, ctx *ProxyCtx) (*ConnectAction, string) {
		if !auth(ctx.Req, f) {
...
func ProxyBasic(proxy *ProxyHttpServer, realm string, f func(user, passwd string) bool) {
